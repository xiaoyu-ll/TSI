...
edge etg[15];
int backtrack[15];
int asc[15];//每个顶点所属组合数 $$
//int tic[15];//每个顶点时间受限的边数
int tvc[15];//记录点的时间受限的边数 $$
int mv[15];//query相应顶点与待查询图中顶点的匹配数量，pl[i]=mv[i]/v_num
//int me[8]={0,10088,16788,19013,8114,25126,7030,6842};
int mt[15];//边的匹配顺序
//int mt[15]={0,5,1,2,6,4,3};
int tm[15];//由order反求边
int match[15];//匹配结果
...
vector< vector<int> > vec(15);//存储候选匹配顶点
int arr[15][5];//存储每一个组合，arr[i][1]为组合的第一个顶点,arr[i][1]为组合的第二个顶点,arr[i][3]为组合的第三个顶点,arr[i][4]为组合中所有顶点都在order中出现的最小位置
int arrc;//记录有多少个组合
...
void readquery()//读查询图的边，顶点标签，边的时序
{
    ...
    cout<<"read query8 successful!"<<endl;
    string a,b;
