...
struct edgenode//enq[15],边结点，enq与eq是一一对应的
{
    int id;
    vector<int> adj;
    vector<int> adje;
...
struct edgeedge//eeq[15],连接边结点的边
{
    int id;
    int from,to;
};
...
struct edgetimecon//etc[15]存储有时间限制的不存在公共顶点的两边
{
...
edge eg[maxn];//
vertex vg[maxn];
equery eq[15];//query的边集
vquery vq[15];//query的点集
edgetimecon etc[15];
edgenode enq[15];//query中的边转化为结点
edgeedge eeq[15];//连接query中相邻的边，类似最小生成树，边数等于结点数减一
edgeedge etq[15];//判断生成树是否有环
int me[15];//query相应边与待查询图中边的匹配数量
//int me[8]={0,10088,16788,19013,8114,25126,7030,6842};
int mt[15];//边的匹配顺序
int tm[15];//由order反求边
int timeconstrain[15][15];
int con_e[15];//记录每条边出现在时间限制组合中的次数
int matche[15];//边匹配结果
int matchv[15];//中间顶点匹配结果
int neibor[15];//形成matchorder的直接邻居
int neibor_can[15];//形成matchorder的候选邻居
int neiborsp[15];//直接邻居的种类，1代表由邻居指向该结点，2代表由该结点指向邻居
int neiborsp_can[15];//直接邻居种类的候选
map<string,string>lg;//账户对应的类型
map<string,string>lq;//query的顶点标签
string label[15];//query顶点的标签
int arrc;
int che[15];
map<int,int> vv;
int arr[15][6];//存储每一个组合，arr[i][1]为组合的第一条边,arr[i][2]为组合的第二,arr[i][3]为组中两条边的时间限制大小,arr[i][4]为组合中所有顶点都在order中出现的最小位置
vector< vector<int> > vec(15);//存储候选匹配边结点
...
void readquery()//读查询图的边，顶点标签，边的时序
{
    ...
    cout<<"read query2 successful!"<<endl;
    string a,b;
    int i=1,j=0,k=0,etci=1,cou=1;//i是边序号
    while(rd>>a>>b)//读每一条边，源结点和终结点
    {
        cout<<a<<" "<<b<<endl;
        if(a=="|")
        break;
...
void readedgelistaa()//读转账数据
{
    ...
    //ifstream rdaa("dataset/to_dataset1.txt");
    if(!rdaa)
    {
        cout<<"error!"<<endl;
        exit(1);
...
void expandmatching(int dep)
{
    ...
    {
        vec[mt[dep]].clear();
        int diradj=tm[neibor[mt[dep]]];//第n条匹配边的直接邻居,mt[n]是第n条匹配边在query中的编号
        int vjoin;
        ...
        else//两边只共用一个顶点
        {
            ...
            else if(emode[1]=='2')//共同顶点是源结点
            {
                ...
                else if(emode[2]=='1')//邻居是小边
                {
                    for(int j=0;j<vg[jointemp].adje1.size();j++)
                    {
                        if(lg[to_string(vg[jointemp].adj1[j])]==lq[eq[mt[dep]].target_id]&&eg[vg[jointemp].adje1[j]].dur>eg[matche[diradj]].dur&&(eg[vg[jointemp].adje1[j]].dur-eg[matche[diradj]].dur)<=timeconstrain[mt[diradj]][mt[dep]]&&vg[vg[isn].adj1[j]].indegree>=vq[itq].indegree)
                        {
                            if(che[itq]!=0&&vv[itq]!=vg[isn].adj1[j])
                            continue;
                            bool flage=true;
